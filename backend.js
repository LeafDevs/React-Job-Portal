...
const secretKey = 'secretKeyLeafDevs123456';

app.use('/api/v1/prism/jobs', verifyToken);
app.use('/api/v1/prism/applications', verifyToken);

app.use('/dashboard', verifyToken);
app.use('/applications/:jobId', verifyToken);
app.use('/applications/:jobId/:applicationId', verifyToken);
...
const verifyToken = (req, res, next) => {
  ...
  try {
    ...
    console.log(`[${getTimestamp()}] Invalid Token`);
...
app.post('/api/v1/prism/auth/login', (req, res) => {
  const { username, password } = req.body;
  const user = users.find(u => u.username === username && u.password === password);
  if (!user) {
    return res.status(400).send('Invalid Credentials');
  }
  const token = jwt.sign({ user_id: user.id, username }, secretKey, { expiresIn: '12h' });
...
app.post('/api/v1/prism/auth/logout', (req, res) => {
  console.log(`[${getTimestamp()}] POST request to '/api/v1/prism/auth/logout'`);
